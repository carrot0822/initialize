...
export default {
  mounted() {
      // 这里替换成vue版本的会舒服的多 不过有svg我就不改了
    let navtab = document.querySelector("nav.navtab"); // 抓取tab组
    ...
    navtabItems.forEach((navtabItem, activeIndex) => {
      navtabItem.addEventListener("click", () => {
        ...
        this.$refs.navtab.style.setProperty("--active-index", `${activeIndex}`); 
      });
    });
  }
};
...
<style lang="scss">
@import url(https://fonts.googleapis.com/css?family=Lato); // 引用字体
...
.navtab {
  --navtab-width: 600px;
  --navtab-item-width: calc(var(--navtab-width) / 4 - 20px); // 空格不能乱加
  --navtab-overlay-width: calc(
    var(--navtab-item-width) + 80px
  ); // 这里的80与20是如何计算的
  --active-index: 0;
  position: relative;
  ...
  ul {
    ...
    .navtab-item {
      ...
      &.active {
        width: var(--navtab-overlay-width); // 颜色变化呢
        svg {
