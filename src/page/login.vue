<template>
  <div id="login">
    <div class="loginBox">
      ...
      <div class="bottonBox">
        <el-button style="width:100%" @click="submit" type="primary">点击登录</el-button>
      </div>
    </div>
    <!-- 验证码 -->
    <div class="validate">
...
export default {
  ...
  methods: {
    ...
    drag(e) {
      ...
      let slider = document.getElementById("sliderBlock"); // canvas滑块DOM
      const downCoordinate = { x: e.x, y: e.y }; // 获取按下的鼠标x轴和y轴
      let check = Number(this.slider.mx) - Number(this.slider.bx); // 滑块与目标的X轴差距
      console.log(check,downCoordinate,'这里的值')
      // x轴数据
      let x = 0;
      // 滑动函数
      const move = moveEV => {
        console.log("moveEV", moveEV.x);
